...
bot = commands.Bot(command_prefix="ㅋ", intents=discord.Intents.all())
access_token = os.environ["BOT_TOKEN"]
status = cycle(['ㅋ도움말로 명령어 확인', '더욱더 발전하겠습니다', '이 메세지는 10초마다 한번씩 바뀝니다', '디스코드서버를 편리하게'])

@bot.event
async def on_command_error(ctx, error):
    if isinstance(error, CommandNotFound):
        embed = discord.Embed(title = ":x:Error!", description = "해당 명령어는 존재하지 않습니다.\nㅋ도움말으로 명령어를 찾아주세요.", color = 0x00FF21)
