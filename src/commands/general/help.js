    ...
    const row = new MessageActionRow()
      ...
      .addComponents(
        new MessageButton()
          ...
          .setStyle('LINK')
          .setURL(
            'https://discord.com/oauth2/authorize?client_id=704999866094452816&permissions=8&scope=bot'
          )
      )
      .addComponents(
        new MessageButton()
    ...
    const Embed = new MessageEmbed()
      ...
      .addFields(
        ...
        },
        {
          name: '일반',
          value:
            '`도움말(help, 도움, 명령어, commands, HELP)`, `문의(support)`, `개발자`, `정보 (info, Info)`',
        },
        {
          name: '관리',
        ...
        },
        {
          name: '유틸리티',
          value:
            '`핑(ping)`, ~~`프로필(proflie, 유저정보, userinfo, (ui)`~~, `랜덤숫자`, ~~`따라해`~~, `업타임(uptime)`',
        },
        {
          name: '봇 개발자 전용 명령어',
          value:
            '`dokdo(dok, Dok, debug, debugging, Dokdo, 독도)`, `eval(Eval, 이발)`',
        }
      )
